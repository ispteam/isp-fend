...
const Suppliers = (props) => {
  ...
  useEffect(() => {
    if(clients.length == 0 && suppliers.length == 0 && requests.length == 0 && brands.length == 0){
  ...
  return (
    <SharedNavLayout navList={generalReducer.moderatorNav} logoLink={"/en/moderator"} footerInnerValue={<p>MODERATOR PAGE</p>}>
        <SuppliersDashboard token={props.token} moderator={true}/>
    </SharedNavLayout>
  );
};
